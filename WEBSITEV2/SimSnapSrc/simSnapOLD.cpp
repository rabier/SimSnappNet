...
void output_xml_SnappNet(ostream& os, const vector<string>& taxa, phylo<basic_newick>& tree, const vector<uint>& sampleSizes, double u, double v, const vector<vector<uint> >&alleleCounts, bool simulationOutput, bool polymorphicOnly, bool initialiseFromTruth, bool snappDominant, bool snappNoMutation, bool useSNAPPMCMC, const string fileroot="test") {
	...
	os<<"\t<data spec='speciesnetwork.SnapData' id='snapalignment' name='alignment' dataType='integer'  statecount='" <<statecount + 1<<"'>\n";
	...
	os<<"\t\t\t<state id='state'>\n";
        os<<"\t\t\t<stateNode id='network:species' spec='speciesnetwork.NetworkParser' tree='@newick:species'>\n";	 
	os<<"\t\t\t</stateNode>\n";
...
os<<"\t<!-- be careful, originTime:species is a scaling factor, if you do not want to scale anything it has to be set to the same value as origin (the height of) the network -->\n";
	...
	os<<"\t\t\t<parameter id='alpha' lower='0.0'  name='stateNode'>1.0</parameter>\n";
	os<<"\t\t\t<parameter id='beta' lower='0.0'  name='stateNode'>200.0</parameter>\n";
	os<<"\t\t\t<parameter id='kappa' lower='0.0'  name='stateNode'>34.6574</parameter>\n";
	os<<"\t\t\t</state>\n\n";
	
        os<<"\t<init id='SNI' spec='speciesnetwork.MySpeciesNetworkInitializerWithoutEmbedding' estimate='false' method='random' speciesNetwork='@network:species' origin='@originTime:species'>\n";
        os<<"\t</init>\n\n";

	os<<"\t<distribution id='posterior' spec='util.CompoundDistribution'>\n";
	os<<"\t\t<distribution id='prior' spec='util.CompoundDistribution'>\n";
	os<<"\t\t\t<distribution id='networkPrior' spec='speciesnetwork.BirthHybridizationModel' network='@network:species' netDiversification='@netDivRate:species' turnOver='@turnOverRate:species' betaShape='1.0'/>\n";
	...
	os<<"\t\t\t<prior id='turnOverPrior' name='distribution' x='@turnOverRate:species'>\n";
        ...
        os<<"\t\t\t</prior>\n";
	os<<"\t\t\t<distribution spec='snaplikelihood.SnAPPriorGH' name='distribution' id='snapprior' kappa='@kappa' alpha='@alpha' beta='@beta' coalescenceRate='@coalescenceRate'/>\n";
	os<<"\t\t</distribution>\n";
	os<<"\t\t<distribution id='likelihood' spec='speciesnetwork.SnAPTreeLikelihoodGH' data='@snapalignment' speciesNetwork='@network:species' mutationRateU='@u'  mutationRateV='@v' coalescenceRate='@coalescenceRate'>\n";
	os<<"\t\t</distribution>\n";
	os<<"\t</distribution>\n\n";
	...
	os<<"\t <operator id='turnOverScale:species' spec='ScaleOperator' parameter='@turnOverRate:species' scaleFactor='0.5' weight='10.0'/>\n";
 	os<<"\t <operator id='gammaProbUniform:species' spec='speciesnetwork.operators.GammaProbUniform' speciesNetwork='@network:species' weight='10.0'/>\n";
	os<<"\t <operator id='gammaProbRndWalk:species' spec='speciesnetwork.operators.GammaProbRndWalk' speciesNetwork='@network:species' weight='10.0'/>\n";
	os<<"\t <operator id='originMultiplier:species' spec='speciesnetwork.operators.OriginMultiplier' speciesNetwork='@network:species' origin='@originTime:species' weight='0.0'/>\n";	
	os<<"\t <operator id='addReticulation:species' spec='speciesnetwork.operators.AddReticulationGH' speciesNetwork='@network:species' weight='10.0' coalescenceRate='@coalescenceRate'/>\n";
	os<<"\t<operator id='deleteReticulation:species' spec='speciesnetwork.operators.DeleteReticulationGH' speciesNetwork='@network:species' weight='10.0'  coalescenceRate='@coalescenceRate'/>\n";
	os<<"\t<operator id='networkMultiplier:species' spec='speciesnetwork.operators.MyNetworkMultiplierWithoutGeneTrees' speciesNetwork='@network:species' origin='@originTime:species' weight='5.0'/>\n";
	os<<"\t <operator id='flipReticulation:species' spec='speciesnetwork.operators.FlipReticulation' speciesNetwork='@network:species' weight='10.0'/>\n";
	os<<"\t <operator id='relocateBranch:species' spec='speciesnetwork.operators.RelocateBranch' speciesNetwork='@network:species' weight='30.0'/>\n";
	os<<"\t <operator id='nodeSlider:species' spec='speciesnetwork.operators.NodeSlider' speciesNetwork='@network:species' origin='@originTime:species' isNormal='true' sigma='0.005' weight='10.0'/>\n";
 	os<<"\t <operator id='NodeUniform:species' spec='speciesnetwork.operators.NodeUniform' speciesNetwork='@network:species' weight='10.0'/>\n";
	os<<"\t <operator id='relocateBranchNarrow:species' spec='speciesnetwork.operators.RelocateBranchNarrow' speciesNetwork='@network:species' weight='10.0'/>\n";
	os<<"\t <operator id='ChangeUAndV' spec='snapp.operators.MutationMover' u='@u' v='@v' window='0.1'  weight='0'/>\n";
	os<<"\t <operator id='ChangeGamma' spec='snapp.operators.GammaMoverGH'  scale='0.5' weight='10' coalescenceRate='@coalescenceRate'/>\n"; 
	os<<"\t <operator id='ChangeAllGamma' spec='snapp.operators.GammaMoveAllGH'  scale='0.5' weight='10' coalescenceRate='@coalescenceRate'/>\n\n";
	...
	os<<"\t<logger id='tracelog' fileName='test_3s.xml.trace.log' logEvery='1000' model='@posterior' sort='smart'>\n";
        ...
        os<<"\t\t <log id='height:species' speciesNetwork='@network:species' spec='speciesnetwork.NetworkStatLogger'/>\n";
	os<<"\t\t <log idref='u'/>\n";
	os<<"\t\t <log idref='v'/>\n";
        os<<"\t </logger>\n";

	os<<"\t<logger id='specieslog' fileName='test_3s.xml.species.trees' logEvery='1000' mode='tree'>\n";
        os<<"\t\t<log id='networkLogger:species' spec='speciesnetwork.NetworkWithMetaDataLoggerGH' speciesNetwork='@network:species' coalescenceRate='@coalescenceRate'/>\n";
	os<<"\t </logger>\n";
