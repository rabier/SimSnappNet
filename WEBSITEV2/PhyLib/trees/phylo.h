 ...
 * @author David Bryant
 * @version 0.1
 * modified by CE Rabier in order to handle networks
 */
...
public:
	...
	public:
		...
		// CE: Same function as above but without detaching the node, in order to handle reticulate node
		...
		iterator graft_reticulate_node(iterator position, iterator node){
			...
			(node.current)->par_right = (position.current);
			node->length_right=nodeToRemove->length;
			node->theta_right=nodeToRemove->theta;				
			(position.current)->left->right=(position.current)->left;
