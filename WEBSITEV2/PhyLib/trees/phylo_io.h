 ...
 * @author David Bryant
 * @version 0.1
 * modified by CE Rabier to handle networks
 */
...
namespace Phylib {
	...
	public:
		...
		//CE
		// fix it : maybe change name of length_right
		double length_right;
	        basic_newick() :
		id(-1), length(0.0), length_right(0.0), meta_data("") {
		}
		basic_newick(int val) :
		id(val), length(0.0), length_right(0.0), meta_data("") {
		}
		basic_newick(int val, double val2) :
		id(val), length(val2), length_right(0.0), meta_data("") {
		}
		basic_newick(int val, double val2, const string& val3):
		id(val), length(val2), length_right(0.0), meta_data(val3) {
		}
		// END CE
	...
	template<typename T> void read_newick(istream& is, phylo<T>& tree, vector<string>& taxa_names, double defaultLength, bool toplevel) { //reads in tree in phylip format.
		...
		// TRY TO SEE IF ICAN write something
		//	printf("\tJe lis mon Newick");
		//ENDTRY
		...
		if (next_tok == io_tree_name) {
			...
			if (at_leaf) {
				...
				//	printf("Voici celle que j avais deja trouve et que je retrouve %d", tree.header().left()->id);
			}
			next_tok = peek_next_tree_token<T>(is);
		} else {
			if (at_leaf) {
